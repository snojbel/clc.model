...
for(s in 1:length(sigma)){
  ...
  for(i in 1:length(two.res.sym$Total.endpoint.CLC.even)){
    this.run <- two.res.sym$Total.endpoint.CLC.even[[i]][[s]]
    this.run$run <- i
    this.run$Adult_sigma <- adu.sigma
    this.run$Juvenile_sigma <- juv.sigma
    last.year.list.even <- rbind(last.year.list.even, this.run)
  }
...
for(s in 1:length(sigma)){
  ...
  for(i in 1:length(two.res.asym$Total.endpoint.CLC.even)){
    this.run <- two.res.asym$Total.endpoint.CLC.even[[i]][[s]]
    this.run$run <- i
    this.run$Adult_sigma <- adu.sigma
    this.run$Juvenile_sigma <- juv.sigma
    last.year.list.even <- rbind(last.year.list.even, this.run)
